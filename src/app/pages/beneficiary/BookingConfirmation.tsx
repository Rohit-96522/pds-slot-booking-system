...
import { CheckCircle, Calendar, Clock, MapPin, Package, Download, Home, Loader2 } from 'lucide-react';
...
export default function BookingConfirmation() {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      ...
      try {
        // Fetch all bookings by current user and find the one matching bookingId
  ...
  const bookId = (booking._id || booking.id)!;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Your slot has been successfully booked</p>
        </div>

        <Card className="mb-6">
          <CardHeader className="bg-blue-50">
            <CardTitle className="text-center text-sm font-mono">Booking ID: {bookId}</CardTitle>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            {/* QR Code */}
            <div className="flex justify-center py-6 bg-white border-2 border-dashed border-gray-300 rounded-lg">
              <QRCodeSVG value={booking.qrCode || bookId} size={200} level="H" includeMargin={true} />
            </div>
            ...
            <div className="border-t pt-6 space-y-4">
              <div>
                ...
                <p className="font-medium text-gray-900 flex items-center gap-2">
                  <MapPin className="h-4 w-4" /> {booking.shopName}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900 flex items-center gap-2">
                    <Clock className="h-4 w-4" /> {booking.timeSlot}
                  </p>
                </div>
              </div>
              <div>
            ...
            <div className="border-t pt-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-50 p-3 rounded-lg">
                  ...
                  <p className="text-lg font-semibold text-blue-700">{booking.entitlement?.rice} kg</p>
                </div>
                <div className="bg-green-50 p-3 rounded-lg">
                  <p className="text-sm text-gray-600">Wheat</p>
                  <p className="text-lg font-semibold text-green-700">{booking.entitlement?.wheat} kg</p>
                </div>
                <div className="bg-yellow-50 p-3 rounded-lg">
                  <p className="text-sm text-gray-600">Sugar</p>
                  <p className="text-lg font-semibold text-yellow-700">{booking.entitlement?.sugar} kg</p>
                </div>
                <div className="bg-purple-50 p-3 rounded-lg">
                  <p className="text-sm text-gray-600">Kerosene</p>
                  <p className="text-lg font-semibold text-purple-700">{booking.entitlement?.kerosene} L</p>
                </div>
              </div>
            </div>
